...
| `app/reports.tsx` | רשימת הדיווחים שנשמרו, מהחדש לישן, עם שכר מחושב לכל דיווח (שלבים 2, 4) |
| `app/workplaces.tsx` | רשימת מקומות העבודה + שכר ונסיעות מצטברים + כפתור הוספה (שלבים 1, 4, 5) |
| `app/workplace-form.tsx` | טופס הוספה / עריכה / מחיקה של מקום עבודה (שלב 1) |
| `app/settings.tsx` | בורר בסיס חישוב השכר (שלב 4), מונה "דקות עבודה מצטברות" ופדיון לשעות אקדמיות (שלב 3); שאר התגיות — שלב 6 |
...
| `lib/pay.ts` | חישוב שכר טהור: שכר לדיווח, שכר מצטבר למקום עבודה, ופירוט לכל המקומות (שלב 4) |
| `lib/payBasis.tsx` | בסיס חישוב השכר השעתי (אקדמיות / רגילות) — הגדרה גלובלית נשמרת מקומית ו־context (שלב 4) |
...
- [x] **שלב 4 — חישוב שכר:** מודול `lib/pay.ts` מחשב שכר לכל דיווח ולכל מקום עבודה —
      שעתי: תעריף × שעות לפי בסיס החישוב; חודשי: סכום קבוע. בורר "בסיס חישוב השכר"
      (שעות אקדמיות / רגילות, ברירת מחדל אקדמיות) במסך ההגדרות, גלובלי ומשפיע גם
      רטרואקטיבית. השכר מוצג בכרטיס הדיווח וברשימת מקומות העבודה.
